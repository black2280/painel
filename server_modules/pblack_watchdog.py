#!/usr/bin/python3
# -*- coding: utf-8 -*-
# pblack_watchdog.py — Keep pblack_module.py alive (cron every minute)
...
# Configuração
MODULE_PORT = 7270  # Pode ler de um arquivo de config se existir
MODULE_PATH = '/root/pblack_module.py'
LOG_PATH = '/root/pblack_watchdog.log'

def log(msg):
    """Escreve no log do watchdog."""
    try:
...
def get_token():
    """Extrai o token de autenticação do pblack_module.py."""
    try:
...
def is_module_running():
    """Verifica se o processo pblack_module.py está rodando."""
    try:
        result = os.popen('pgrep -f "python3.*pblack_module.py"').read().strip()
        return bool(result)
    except Exception:
        return False
...
def restart_module():
    ...
    os.system('pkill -f "python3.*pblack_module.py" 2>/dev/null || true')
    os.system('pkill -f pblack_module.py 2>/dev/null || true')
    ...
    result = os.system(f'nohup python3 {MODULE_PATH} > /root/pblack_module.log 2>&1 &')
    if result == 0:
...
def ensure_cron():
    """Garante que o watchdog está no crontab."""
    cron_entry = "* * * * * python3 /root/pblack_watchdog.py"
    result = os.popen('crontab -l 2>/dev/null').read()
    
    if 'pblack_watchdog.py' not in result:
        log("Adicionando watchdog ao crontab")
        ...
        if cron_entry not in result:
            ...
            lines = [l for l in result.split('\n') if 'pblack_watchdog.py' not in l]
