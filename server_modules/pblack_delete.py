#!/usr/bin/python3
# -*- coding: utf-8 -*-
# pblack_delete.py — Batch delete users from SSH server
...
if len(sys.argv) != 2:
    print("Uso: python3 pblack_delete.py <arquivo>")
    sys.exit(1)
...
for linha in linhas:
    ...
    if len(cols) >= 2:
        ...
        if primeiro_eh_uuid:
            ...
            os.system(f"/root/pblack_agent v2raydel {cols[0]} {cols[1]}")
        elif segundo_eh_uuid:
            # Formato: username uuid
            os.system(f"/root/pblack_agent v2raydel {cols[1]} {cols[0]}")
        else:
            # Não identificou UUID, assume que é apenas username SSH
            os.system(f"/root/pblack_agent removessh {cols[0]}")
    else:
