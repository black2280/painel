jean
# =============================================================
#  Painel Black — Instalador v4
#  Repositório: https://github.com/black2280/painel-black
# =============================================================

RED='\033[0;31m'; GREEN='\033[0;32m'; YELLOW='\033[1;33m'
BLUE='\033[0;34m'; CYAN='\033[0;36m'; BOLD='\033[1m'; NC='\033[0m'

INSTALL_DIR="/opt/painel-black"
SERVICE_NAME="painel-black"
...
uninstall_existing() {
    ...
    pkill -9 -f "painel-black"   2>/dev/null || true
    sleep 2
...
copy_files() {
    ...
    if [ -d "$INSTALL_DIR/.git" ]; then
        ...
        git reset --hard origin/main 2>/dev/null || \
            git reset --hard origin/black 2>/dev/null || \
...
# ── Instala painel ─────────────────────────────────────────────
install_panel() {
    banner "Instalando Painel Black..."
    copy_files
...
[Unit]
Description=Painel Black SSH Manager
...
IPCNF
    ...
    if [ ! -f /etc/nginx/ssl/painel-selfsigned.crt ]; then
        ...
        openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
            ...
            -subj "/CN=painel-black/O=PainelBlack/C=BR" 2>/dev/null
    fi
...
print_info() {
    ...
    echo ""
    echo -e "${CYAN}╔══════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║       PAINEL BLACK — INSTALACAO CONCLUIDA       ║${NC}"
    echo -e "${CYAN}╚══════════════════════════════════════════════════╝${NC}"
    echo ""
...
clear
banner "╔══════════════════════════════════════╗"
banner "║      PAINEL BLACK — INSTALADOR      ║"
banner "╚══════════════════════════════════════╝"
echo ""
