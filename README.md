...
```bash
curl -sSL https://raw.githubusercontent.com/black2280/painel/main/install.sh | bash
```

Ou manualmente:
```bash
git clone https://github.com/black2280/painel-black
cd painel-black
bash install.sh
```
...
```bash
bash <(curl -sSL https://raw.githubusercontent.com/black2280/painel-black/main/server_modules/checkuser_install.sh)
```
...
```bash
# Ver logs
journalctl -u painel-black -f

# Reiniciar painel
systemctl restart painel-black

# Backup manual
cd /opt/painel-black && source venv/bin/activate
