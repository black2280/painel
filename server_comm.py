...
def create_ssh_user_on_server(ip: str, port: int, auth_token: str,
    ...
    if uuid:
        cmd = f"/root/pblack_agent v2rayadd {uuid} {username} {password} {days} {limit}"
    else:
        cmd = f"/root/pblack_agent createssh {username} {password} {days} {limit}"
    ok, out = send_command(ip, port, auth_token, cmd)
...
def delete_user_on_server(ip: str, port: int, auth_token: str,
    ...
    if uuid:
        cmd = f"/root/pblack_agent v2raydel {uuid} {username}"
    else:
        cmd = f"/root/pblack_agent removessh {username}"
...
def renew_user_on_server(ip: str, port: int, auth_token: str,
    ...
    cmd = f"/root/pblack_agent timedata {username} {days}"
    ok, out = send_command(ip, port, auth_token, cmd)
...
def create_test_user_on_server(ip: str, port: int, auth_token: str,
    ...
    if uuid:
        cmd = (f"/root/pblack_agent v2rayaddteste {uuid} {username} {password} {minutes} {limit} "
               f"2>/dev/null || /root/dragonmodule v2rayaddteste {uuid} {username} {password} {minutes} {limit}")
    else:
        cmd = (f"/root/pblack_agent createsshteste {username} {password} {minutes} {limit} "
...
def sync_users_to_server(ip: str, port: int, auth_token: str, users: list, force: bool = False) -> tuple:
    ...
    for u in users:
        ...
        if uuid:
            # pblack_agent v2rayadd espera: uuid username password days limit
            lines.append(
                f"{uuid} {u['username']} {u['password']} {days_left} {u['connection_limit']}")
        else:
            # SSH puro: username password days limit (4 campos)
            lines.append(
    ...
    for idx, batch in enumerate(batches):
        ...
        if is_last:
            ...
            cmd = (
                ...
                f"(python3 /root/pblack_sync.py /tmp/pmg_sync.txt 2>/dev/null || "
            ...
            cmd = (
                ...
                f"    cols = linha.split()\n"
                f"    if len(cols) >= 5:\n"
                f"        # Verifica se o primeiro campo parece um UUID (usuário V2Ray)\n"
                f"        is_v2ray = bool(re.match(r'[0-9a-f]{{8}}-([0-9a-f]{{4}}-){{3}}[0-9a-f]{{12}}', cols[0].lower()))\n"
                f"        if is_v2ray:\n"
                f"            # V2Ray: uuid, username, password, days, limit\n"
                f"            os.system('/root/pblack_agent v2rayadd {{}} {{}} {{}} {{}} {{}} 2>/dev/null || /root/dragonmodule v2rayadd {{}} {{}} {{}} {{}} {{}} 2>/dev/null'.format(*cols[:5], *cols[:5]))\n"
                f"        else:\n"
                f"            # SSH: username, password, days, limit\n"
                f"            os.system('/root/pblack_agent createssh {{}} {{}} {{}} {{}} 2>/dev/null || /root/dragonmodule createssh {{}} {{}} {{}} {{}} 2>/dev/null'.format(*cols[:4], *cols[:4]))\n"
                f"    elif len(cols) >= 4:\n"
                f"        # SSH puro (fallback)\n"
                f"        os.system('/root/pblack_agent createssh {{}} {{}} {{}} {{}} 2>/dev/null || /root/dragonmodule createssh {{}} {{}} {{}} {{}} 2>/dev/null'.format(*cols[:4], *cols[:4]))\n"
...
def install_modules_ssh(ip: str, root_user: str, root_password: str, auth_token: str) -> tuple:
    ...
    files_to_upload = [
        ('pblack_module.py',  '/root/pblack_module.py'),
        ('pblack_agent',      '/root/pblack_agent'),
        ('pblack_sync.py',    '/root/pblack_sync.py'),
        ('sincronizar.py',     '/root/sincronizar.py'),      # legacy compat
        ('pblack_delete.py',  '/root/pblack_delete.py'),
        ('pblack_watchdog.py','/root/pblack_watchdog.py'),
    ...
    try:
        ...
        for local_name, remote_path in files_to_upload:
            ...
            if not os.path.exists(local_path):
                continue
            if local_name == 'pblack_module.py':
        ...
        commands = [
            "chmod +x /root/pblack_agent",
            "chmod +x /root/pblack_module.py",
            ...
            (
                ...
                "printf '\\n# PMG_VPN_BLOCK - Gerado pelo Painel Black\\n"
                "Match Group pmg_vpn\\n"
                "    PermitTTY no\\n"
                "    X11Forwarding no\\n"
            ...
            # ─────────────────────────────────────────────────────────────────
            "pkill -f pblack_module.py || true",
            "nohup python3 /root/pblack_module.py > /root/pblack_module.log 2>&1 &",
            "(crontab -l 2>/dev/null | grep -v pblack_watchdog; echo '* * * * * python3 /root/pblack_watchdog.py') | crontab -",
        ]
        for cmd in commands:
